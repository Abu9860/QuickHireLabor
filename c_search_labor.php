...
if ($result->num_rows > 0) {
    ...
    // If address column doesn't exist, provide default locations or empty array
    $locations = [];
    
    // Optionally, you can add some default locations if needed
    // $locations = [
    //     ['address' => 'Downtown'],
    //     ['address' => 'Uptown'],
    //     ['address' => 'Midtown']
...
$tableExists = $conn->query("SHOW TABLES LIKE 'skills'")->num_rows > 0;

if ($tableExists) {
    // Original query if table exists
    $skills = $conn->query("SELECT name FROM skills")->fetch_all(MYSQLI_ASSOC);
} else {
    // Provide default skills if table doesn't exist
    $skills = [
        ['name' => 'Plumbing'],
        ['name' => 'Electrical'],
        ['name' => 'Carpentry'],
